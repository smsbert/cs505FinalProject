...
public class API {
	...
	System.out.println("TIMES: "+time1+", "+time2+", "+time3);
        OrientDB orientdb = new OrientDB("remote:localhost", OrientDBConfig.defaultConfig());

        // open database session
        try {
	     ODatabaseSession db = orientdb.open(dbname, login, password);
	     
	     OResultSet first15sec = db.query("select count(*) as patientCount, zipcode from Patient where dateTime between ? and ? and statusCode != 0 and statusCode != 1 and statusCode != 3 and statusCode != 4 group by zipcode", time3, time2);
             OResultSet second15sec = db.query("select count(*) as patientCount, zipcode from Patient where dateTime between ? and ? and statusCode != 0 and statusCode != 1 and statusCode != 3 and statusCode != 4 group by zipcode", time2, time1);
	     
	     OResultSet third = db.query("select * from Patient");
	     System.out.println(first15sec.elementStream().count());
	     System.out.println(second15sec.elementStream().count());
             System.out.println(third.elementStream().count());
	     ...
	     while (second15sec.hasNext()) {
                 ...
                 patientCount2 = 0;
		 System.out.println("Try ZIP2, COUNT2:\t" + zipcode2);
                 while (first15sec.hasNext()) {
                     OResult item1 = first15sec.next();
                     zipcode1 = item1.getProperty("zipcode");
                     patientCount1 = item1.getProperty("patientCount");
                     System.out.println("ZIP1, COUNT1: " + zipcode1 + ", " + patientCount1);
		     System.out.println("ZIP2, COUNT2: " + zipcode2 + ", " + patientCount2); 
